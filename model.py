...
class DNN:
    ...
    def train(self, X_train, Y_train, layer_dims, epoch=1_000, learning_rate=0.03, minibatch_size=64):
        layer_dims.insert(0, X_train.shape[0])
        layer_dims.append(
            1 if self.binary_classification else Y_train.shape[0])
        ...
        for i in range(epoch):
            for batch_idx in range(0, int(X_train.shape[1] / minibatch_size)):
                ...
                # Backpropagation for calculating gradients and updatind parameters
    ...
    def forward_prop(self, parameters, X):
        ...
        for l in range(1, len(self.layer_dims)):
            ...
            if l < len(self.layer_dims) - 1:
                ...
                forward_vars["Z" + str(l)] = self.softmax(forward_vars["Z" + str(l)])
    ...
    def backward_prop(self, parameters, forward_vars, Y):
        ...
        for l in range(L, 0, -1):
            ...
            if l == L and self.binary_classification:
                ...
                relu_derivatives = forward_vars["A" + str(l)] > 0
                gradients["dZ" + str(l)] = np.multiply(gradients["dA" + str(l)], relu_derivatives)
